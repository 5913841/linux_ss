...
extern asmlinkage void handle_sec(void);
...
#ifdef CONFIG_SHADOW_STACK
asmlinkage void do_sec(unsigned long ret_ptr)
{
	unsigned long estat = read_csr_estat();
	unsigned long excode = (estat & CSR_ESTAT_EXC) >> CSR_ESTAT_EXC_SHIFT;
	unsigned long sub_excode = (estat & CSR_ESTAT_ESUBCODE) >> CSR_ESTAT_ESUBCODE_SHIFT;
	
	if(excode == EXCCODE_SE){
		switch (sub_excode)
		{
		case EXCSUBCODE_SS_OVERFLOW:
			asm volatile("b handle_ss_overflow");
			break;
		case EXCSUBCODE_SS_UNDERFLOW:
			asm volatile("b handle_ss_underflow");
			break;
		case EXCSUBCODE_SS_ILLEGAL:
			asm volatile("b handle_ss_illegal");
			break;
		default:
		...
		asm volatile(".globl exit_handle_sec");
		asm volatile("exit_handle_sec:");
		asm volatile("jirl $r0, %0, 0" : :"r"(ret_ptr));
	}
}
#endif
...
#ifdef CONFIG_SHADOW_STACK
	set_handler(EXCCODE_SE * VECSIZE, handle_sec, VECSIZE);
#endif
