...
asmlinkage void context_switch_ss(struct task_struct *prev, struct task_struct *next, unsigned long ret_ptr)
{
    ...
    memcpy(prev->thread.shadow_stack_buffer, stack_buffer, top * sizeof(unsigned long));
    unsigned long live_top = r_scfg_d(0x28);
    for(int i = live_top - 8 ; i >= 0 ; i -= 8){
        prev->thread.shadow_stack_buffer[prev->thread.shadow_stack_top + i] = r_scfg_d(SHADOW_STACK_BASE - i);
        safe_pop();
    }
    ...
    memcpy(stack_buffer, next->thread.shadow_stack_buffer, top * sizeof(unsigned long));
    for(int i = 0 ; i < left ; i += 8){
        safe_push(next->thread.shadow_stack_buffer[top + i]);
    }
...
void handle_ss_overflow(void)
{
    ...
    for(int i = size-8 ; i >= 0 ; i-=8){
        stack_buffer[top + i] = r_scfg_d(SHADOW_STACK_BASE - i);
        safe_pop();
    }
    top += size;
